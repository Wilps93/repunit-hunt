#!/bin/bash
# ============================================================================
#  АРХИВ: полная валидация ВЕРСИИ 1 статьи (paper_*_v1.tex, analysis.py).
#  Для текущей версии 3 используйте validate.sh.
#
...
rm -rf figs results.txt paper_ru_v1.pdf paper_en_v1.pdf results_seed*.txt
...
run "build_paper.sh -> paper_ru_v1.pdf, paper_en_v1.pdf" bash build_paper.sh paper_ru_v1 paper_en_v1
...
run "ГОСТ Р 7.0.7-2021"                    python3 check_gost.py paper_ru_v1.tex
...
for doc in paper_ru_v1 paper_en_v1; do
...
pdftotext -enc UTF-8 paper_ru_v1.pdf "$TMP"/f.txt 2>/dev/null
...
pdftotext -enc UTF-8 paper_en_v1.pdf "$TMP"/e.txt 2>/dev/null
...
grep -qE 'УДК|Научная статья|encaptab|Список источников|otherlanguage' paper_en_v1.tex \
  && bad "gost-leak" || ok
...
subprocess.run(["detex","paper_ru_v1.tex"],stdout=open(tmp,"w"),
...
np=$(grep -o 'textcolor{red}' paper_ru_v1.tex | wc -l)
...
inpaper=$(grep -oP 'X_\{\\mathrm\{ver\}\}=\K[0-9\\,]+' paper_ru_v1.tex | head -1 | tr -d '\\,')
